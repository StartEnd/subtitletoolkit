...
function microdvdFrameToMs(frame: number, fps: number) {
  return Math.round((frame / fps) * 1000);
}

function lrcTimestampToMs(timestamp: string) {
  const [minutes, secondsPart] = timestamp.replace(',', '.').split(':');
...
export function parseMicrodvd(input: string): SubtitleCue[] {
  const normalized = normalizeSubtitleInput(input);
  if (!normalized) {
    return [];
  }
  ...
  return rows
    .map((match) => {
      ...
      const declaredFps = body.replace(',', '.');

      if (startFrame <= 1 && endFrame <= 1 && /^\d+(?:\.\d+)?$/.test(declaredFps)) {
        const nextFps = Number(declaredFps);
        if (nextFps > 0 && nextFps <= 120) {
          fps = nextFps;
        }
        return null;
      }
      ...
      const start = microdvdFrameToMs(startFrame, fps);
      const end = microdvdFrameToMs(endFrame, fps);
...
export function parseLrc(input: string): SubtitleCue[] {
  const normalized = normalizeSubtitleInput(input);
  if (!normalized) {
    return [];
  }
  ...
  const rows = normalized
    ...
    .flatMap((line) => {
      ...
      return matches.map((match) => ({
        start: lrcTimestampToMs(match[1]),
        text,
      }));
    })
  ...
  return rows.map((row, index) => {
    const nextStart = rows[index + 1]?.start ?? row.start + 3000;

    return {
      start: row.start,
      end: Math.max(row.start + 1, nextStart),
      text: [cleanSubtitleLine(row.text)],
    };
  });
}
...
export function parseMpl2(input: string): SubtitleCue[] {
  const normalized = normalizeSubtitleInput(input);
  if (!normalized) {
    return [];
  }
  ...
  return normalized
    ...
    .map((line) => {
      ...
      const text = match[3]
        .split('|')
        .map((part) => cleanSubtitleLine(part))
        .filter(Boolean);
