...
seoTitle: Fix Subtitle Sync After a Cut - Shift One Section
description: Repair subtitle sync after a scene cut by shifting only the affected SRT, VTT, ASS, or SSA time range while synced sections stay unchanged.
seoDescription: Fix subtitles after a scene cut online. Shift only the affected SRT, VTT, or ASS range locally with no upload while synced sections stay unchanged.
pubDate: 2026-05-10
...
tags:
  ...
  - shift subtitles after cut
  - fix subtitles after trimming video
...
If the beginning is correct but subtitles go wrong after a cut, use the [Partial Subtitle Shifter](/tools/partial-subtitle-shifter/) on the affected section instead of shifting the whole file. This preserves the synced intro and ending while fixing only the problematic range.
...
If the amount keeps changing gradually after the cut, read [why subtitles drift out of sync](/guides/why-subtitles-drift-out-of-sync/) before applying a fixed offset. Progressive drift usually needs a different correction than a single scene-cut shift.
