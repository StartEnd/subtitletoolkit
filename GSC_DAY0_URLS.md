...
Latest production gate: `pnpm verify:gsc:submit-ready` passed on 2026-06-02 against the live `af957a6` deployment (`sitemap-0.xml` lastmod `2026-06-02T01:43:56.000Z`) using the same local verifier commit. The primary queue below has 21 URL Inspection requests; the current queue has 118 URLs. Keep them unchecked until the manual Search Console requests are actually submitted.
