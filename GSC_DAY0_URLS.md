...
Latest production gate: `pnpm verify:gsc:submit-ready` passed on 2026-06-02 against the live `24f8497` deployment (`sitemap-0.xml` lastmod `2026-06-02T10:53:26.000Z`) using the same local verifier commit. The primary queue below has 21 URL Inspection requests; the current queue has 157 URLs. Keep them unchecked until the manual Search Console requests are actually submitted.
