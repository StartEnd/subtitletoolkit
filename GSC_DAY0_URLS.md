...
Latest production gate: `pnpm verify:gsc:submit-ready` passed on 2026-06-02 against the live `77a4a65` deployment (`sitemap-0.xml` lastmod `2026-06-01T21:33:56.000Z`) using the same local verifier commit. The primary queue below has 21 URL Inspection requests; keep it unchecked until the manual Search Console requests are actually submitted.
